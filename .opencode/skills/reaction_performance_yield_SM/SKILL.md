---
name: reaction_performance_yield_Suzuki_Miyaura
description:  输入一个包含多个Suzuki-Miyaura反应条目的smiles的文件，对所条目进行产率预测，并输出预测结果
license: MIT
---
...
```bash
source "$(conda info --base)/etc/profile.d/conda.sh" && conda activate rxngraphormer && python .opencode/skills/reaction_performance_yield_SM/scripts/performance.py --input_file {{input_file}} --output_path {{output_path}}
```
