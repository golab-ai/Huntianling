...
class TestGmxProc(BaseTestCase):
    ...
    def test_01_recommend_pdb(self):
        ...
        code, out, err = self.run_opencode(prompt, timeout=300)
    ...
    def test_03_download_pdb(self):
        ...
        code, out, err = self.run_opencode(prompt, timeout=120)
    ...
    def test_04_pdb_prepare(self):
        ...
        code, out, err = self.run_opencode(prompt, timeout=240)
        self.save_stdout_stderr(out, err, "runjob/pdb")

        self.assertFileExists("./runjob/pdb/2.pdb")

    @test_level(0)
    def test_05_run_gmx_md(self):
        code, out, err = self.run_opencode(
            "运行MD模拟（1000 步；输入 ./example/pdb/protein_A_apo.pdb；输出到 ./runjob/md）", 
            timeout=300)
        self.save_stdout_stderr(out, err, "runjob/md")
        self.assertFileExists("./runjob/md/protein/job.sh")
        # keywords = """           Step           Time
        #    1000        2.00000"""
    ...
    def test_06_pocket_prediction(self):
        prompt = "预测结合口袋（输入 .example/pdb/protein_A_apo.pdb；输出到 ./runjob/pocket/）"
        code, out, err = self.run_opencode(prompt,  timeout=300)
    ...
    def test_07_molecule_generation(self):
        """环境不对，还得改"""
        prompt = "分子生成（蛋白 + 口袋；输出到 ./runjob/generation）, 输入：./runjob/pdb/2.pdb ./runjob/pocket/pockets_summary.csv"
        code, out, err = self.run_opencode(prompt,  timeout=300)
    ...
    def test_09_docking(self):
        prompt = """分子对接（蛋白 + 小分子；输出到 ./runjob/docking）
输入：
./example/pdb/protein_A_apo.pdb
./example/lig_prep/lig_prepped.sdf
        """
        code, out, err = self.run_opencode(prompt, timeout=600)
    ...
    def test_10_fep(self):
        prompt = """做 RBFE计算，文件输出到 ./runjob/fep
输入：
./example/6B8O-prepare.pdb
./example/8-pose1.sdf"""
        code, out, err = self.run_opencode(prompt, timeout=240)
    ...
    def test_11_admet(self):
        ...
        self.assertFileExists("./runjob/admet/admet_predictions.csv")
...
if __name__ == "__main__":
    ...
    ##remain: 
    # test_04_pdb_prepare
    # test_06_pocket_prediction
    # test_07_molecule_generation X
    # test_09_docking
    # test_10_fep
    # test_11_admet
    # test_12_synthesis