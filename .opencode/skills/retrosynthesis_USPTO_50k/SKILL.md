---
name: retrosynthesis_USPTO_50k
description:  输入一个包含多个化合物的smiles的文件，对所有的分子进行逆合成分析，模型基于USPTO_50k数据集训练
license: MIT
---
...
```bash
source "$(conda info --base)/etc/profile.d/conda.sh" && conda activate rxngraphormer && python .opencode/skills/retrosynthesis_USPTO_50k/scripts/retrosynthesis.py --input_file {{input_file}} --output_path {{output_path}}
```
